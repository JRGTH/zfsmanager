...
if ($in{'pool'}) {
	...
	if ($status{0}{status} or $status{0}{action} or $status{pool}{see}) {
		...
		if ($status{0}{status}) {
			print &ui_table_row("Status:", $status{0}{status});
		}
		if ($status{0}{action}) {
			print &ui_table_row("Action:", $status{0}{action});
		}
		if ($status{0}{see}) {
			print &ui_table_row("See:", $status{0}{see}); 
		}
		print &ui_table_end();
	}
	...
	if ($config{'pool_properties'} =~ /1/) {
		if ($status{0}{scan} =~ /scrub in progress/) {
			print &ui_table_row('Scrub ',"<a href='cmd.cgi?cmd=scrub&stop=y&pool=$in{pool}'>Stop scrub</a>");
		} else {
			print &ui_table_row('Scrub ', "<a href='cmd.cgi?cmd=scrub&pool=$in{pool}'>Scrub pool</a>");}
		...
		if ($config{'pool_destroy'} =~ /1/) {
			print &ui_table_row("Destroy ", "<a href='cmd.cgi?cmd=pooldestroy&pool=$in{pool}'>Destroy this pool</a>");
		}
		print &ui_table_end();
	...
	if ($in{'zfs'}) {
		...
		if ($config{'zfs_properties'} =~ /1/) {
			...
			if ($hash{$in{'zfs'}}{origin}) {
				print &ui_table_row("Promote: ", "This file system is a clone, <a href='cmd.cgi?cmd=promote&zfs=$in{zfs}'>promote $in{zfs}</a>");
			}
		}
		if ($config{'zfs_destroy'} =~ /1/) {
			print &ui_table_row("Destroy: ", "<a href='cmd.cgi?cmd=zfsdestroy&zfs=$in{zfs}'>Destroy this file system</a>");
		}
		print &ui_table_end();
	...
	if ($in{'snap'}) {
		...
		if ($config{'snap_destroy'} =~ /1/) {
			print &ui_table_row('Destroy:',"<a href='cmd.cgi?cmd=snpdestroy&snapshot=$in{snap}'>Destroy snapshot</a>", );
		}
		print &ui_table_end();
